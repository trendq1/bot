...
function barsChart(el, rows) {
  ...
  el.innerHTML = `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Доходы и расходы по дням">
    ${ticks.map((t) => `<line x1="${pl}" x2="${W - pr}" y1="${y(t)}" y2="${y(t)}" stroke="#262a33"/><text x="${pl - 6}" y="${y(t) + 4}" fill="#7d8290" font-size="10" text-anchor="end">${Math.round(t)}$</text>`).join("")}
...
VIEWS.overview = async () => {
  ...
  $("view").innerHTML = `
    ...
    <div class="grid cols2">
      <div class="card"><h3>Движок</h3>${d.engine ? `<div class="muted small">процессов клиентов: ${d.engine.workers} · ИИ: ${d.engine.ai_enabled ? "подключён" : "нет ключа — алгоритм"}</div>` +
        table(["Монета", "Цена", "Режим", "Источник", "Анализ"], d.engine.symbols.map((s) => `<tr><td>${s.symbol}</td><td class="num">${s.price ?? "—"}</td><td>${REGIME[s.regime] || "—"}</td><td>${s.source === "ai" ? "ИИ" : s.source ? "алгоритм" : "—"}</td><td>${s.ai_age_min != null ? s.ai_age_min + " мин назад" : "—"}</td></tr>`))
        : `<div class="muted">Торговый движок выключен (Настройки → Торговля)</div>`}</div>
...
// ───────────── клиенты ─────────────
VIEWS.users = async () => {
  $("headActions").innerHTML = `<a class="btn ghost" href="/admin/api/export/users.csv">⬇ CSV</a>`;
...
VIEWS.payments = async () => {
  $("headActions").innerHTML = `<a class="btn ghost" href="/admin/api/export/payments.csv">⬇ CSV</a>`;
...
VIEWS.finance = async () => {
  ...
  const t = d.totals;
  $("headActions").innerHTML = `<select id="finDays">${[7, 30, 90, 365].map((x) => `<option value="${x}" ${x === days ? "selected" : ""}>${x} дней</option>`).join("")}</select><a class="btn ghost" href="/admin/api/export/finance.csv">⬇ CSV</a>`;
...
// ───────────── сделки ─────────────
VIEWS.trades = async () => {
  $("headActions").innerHTML = `<a class="btn ghost" href="/admin/api/export/trades.csv">⬇ CSV</a>`;
  $("view").innerHTML = `<div class="card"><div class="row">
...
VIEWS.ai = async () => {
  ...
  const syms = d.engine ? d.engine.symbols.map((s) => s.symbol) : Object.keys(tun);
  $("view").innerHTML = `
    <div class="card"><h3>Рынок сейчас ${d.engine ? `<span class="muted small">· ИИ ${d.engine.ai_enabled ? "подключён" : "не подключён (нужен ключ Anthropic)"}</span>` : ""}</h3>
...
VIEWS.settings = async () => {
  ...
  $("headActions").innerHTML = `<button class="btn danger" id="restart">↻ Перезапустить приложение</button>`;
  $("restart").onclick = async () => {
    if (!confirm("Перезапустить? Торговые процессы клиентов остановятся на ~10 секунд и запустятся снова.")) return;
    await post("/system/restart"); toast("Перезапуск…");
    const wait = async () => { try { await api("/me"); toast("Приложение снова работает"); } catch (e) { setTimeout(wait, 2000); } };
  ...
  <p class="muted small">Секретные ключи хранятся в базе в зашифрованном виде и показываются маской. Чтобы заменить ключ — сотрите маску и вставьте новый.</p>`;
  $("saveSet").onclick = async () => {
    ...
    try {
      ...
      $("setRes").textContent = "Сохранено." + (r.restart_required.length ? " Для применения «" + r.restart_required.join("», «") + "» нажмите «Перезапустить приложение»." : "");
